...
describe('dashboard bots page', () => {
  ...
  it('renders the honest empty state when no bots are injected', () => {
    renderBotsPage();
    const bots = screen.getByRole('region', { name: 'Your bots' });
    expect(within(bots).getByText('No data yet')).toBeTruthy();
    expect(screen.getByText('No bots yet — describe your first bot.')).toBeTruthy();
    expect(document.body.textContent).not.toContain('(example)');
    expect(consoleError).not.toHaveBeenCalled();
  });
...
describe('dashboard bot cards — links to the detail page', () => {
  ...
  it('omits the counts line for live rows with no server counts', () => {
    ...
    expect(items[0].textContent).not.toContain('members');
    expect(items[0].textContent).not.toContain('Published');
    expect(consoleError).not.toHaveBeenCalled();
  });
});
...
describe('dashboard filtering and sorting on the bots page', () => {
  ...
  it('shows the honest empty state when there are no bots', () => {
    ...
    const describe = screen.getByRole('link', { name: 'Describe your first bot' });
...
describe('dashboard bots page — live binding', () => {
  ...
  it('shows the honest empty state when the live list answers 401', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({
        ok: false,
    ...
    /* KI-030: failure is empty, not example — no mock rows as the account's. */
    expect(await screen.findByText('No bots yet — describe your first bot.')).toBeTruthy();
