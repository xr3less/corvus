...
- [ ] Tidy commit (grab install + box reports + living docs; tree clean; push via founder `! git push`)
- [ ] KI-030 honesty pass — paid CTAs disabled or wired to real checkout, no fake Pro pill / hardcoded credits, empty states not mock Pro, token FAQ matches custody, Publish ≠ live on Discord, footer honest. Close when: a stranger sees no paid promise the product cannot keep.
