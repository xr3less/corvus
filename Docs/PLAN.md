...
- [ ] Deploy to a shareable URL (pipeline ready 2026-09-15 — IP-first unblocked D-135, no domain needed; needs server secrets + first real SSH run; no auto-deploy on push until founder approves)
- [x] IP-first prep DONE 2026-09-18 (D-135: Caddy TLS via `13-140-181-113.nip.io`, RUNBOOK §7 rewritten, APP_URL pattern locked; box provisioned ALL DONE, commit pending push)
